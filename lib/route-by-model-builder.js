...
async function routeByModelBuilder ({ file, dir, pathPrefix, ns, parent }) {
  ...
  for (const method of methods) {
    ...
    if (customMod.schema === true) customMod.schema = await this.docSchemaModel({ model: builder.model, method, options: { hidden: builder.hidden } })
    ...
    merge(mod, customMod, {
      ...
      handler: async function (req, reply) {
        ...
        const data = await helper({ model: builder.model, req, reply, options, transaction: builder.transaction })
        data.success = true
