...
async function routeByModelBuilder ({ file, ctx, dir, pathPrefix, ns, parent }) {
  ...
  const { readConfig, parseObject } = this.app.bajo
  const { isSet } = this.lib.aneka
  const { methodMap, getParams } = this.app.waibuDb
  const { getInfo } = this.app.dobo
  const { isFunction, camelCase, omit, merge, keys, filter } = this.lib._
  ...
  let url = path.dirname(file).replace(`${dir}/${pathPrefix}`, '').replaceAll('@', ':')
  ...
  const method = 'GET'
  const disabled = schema.disabled.includes(method) || builder.disabled.includes(method)
  ...
  const mod = {
    ...
    handler: async function (req, reply) {
      ...
      const helper = me.app.waibuDb[camelCase(`stat ${req.params.stat}`)]
