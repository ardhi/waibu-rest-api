...
async function routeByModelBuilder ({ file, ctx, dir, pathPrefix, ns }) {
  const { getPlugin } = this.app.bajo
  getPlugin('dobo') // ensure dobo is loaded
  const { readConfig, parseObject, isSet } = this.app.bajo
  const { methodMap, getParams } = this.app.waibu
  ...
  const me = this
  const mods = []
  const swagger = this.app.waibuSwagger
  for (const method of methods) {
    ...
    if (!customMod.schema && swagger) customMod.schema = await swagger.docSchemaModel({ model: builder.model, method, ctx, options: { hidden: builder.hidden } })
    ...
    merge(mod, customMod, {
      ...
      handler: async function (req, reply) {
        const helper = me.app.waibu[camelCase(`record ${method === 'replace' ? 'update' : method}`)]
